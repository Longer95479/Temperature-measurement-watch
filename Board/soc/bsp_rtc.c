...
//从时间戳转化为北京时间并更新到时间结构体 
/*例如 get_time(RTC_GetCounter(),&systmtime); 将读取RTC计数值并转换为北京时间以更新time结构体*/
void get_time(uint32_t TimeVar,struct rtc_time *tm)
{
	   uint32_t BJ_TimeVar;	
	   /*  把标准时间转换为北京时间*/
...
//将时间结构体成员更新到时间戳
void update_time(struct rtc_time *tm,int hour, int min, int sec)
{
	tm->tm_hour =hour;
	tm->tm_min =min;
	 tm->tm_sec=sec;
